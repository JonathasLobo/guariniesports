...
if (id) {
    ...
    .then((results) => {
        ...
        const {pickRate, winRate, enemyPokemons} = pokemonInfo

        let allyKeys = Object.keys(enemyPokemons).filter(alliedPokemon => enemyPokemons[alliedPokemon].pickRate >= 1)
        ...
        rightPokeballImg.src = './images/icons/pokeball.png';
        ...
        rightPlayernameSpan.innerText = `Rendimento`
        rightPlayernameSpan.onclick = () => {
            window.location.href = window.location.pathname.replace('rendimento-result.html', `pokemon-result.html?id=${id}&pokemon=${pokemonName}`);
        }
        ...
        bestWRTh.innerText = 'DIFICULDADE CONTRA'
        ...
        bestWRWinRateTh.innerText = 'WR INIMIGA'
        bestWRHeader.appendChild(bestWRWinRateTh)

        for (const pokemonName of finalSortedWinKeys) {
            const {pickRate, winRate, isUp} = enemyPokemons[pokemonName]
        ...
        worstWRTh.innerText = 'FACILIDADE CONTRA'
        ...
        worstWRWinRateTh.innerText = 'WR INIMIGA'
