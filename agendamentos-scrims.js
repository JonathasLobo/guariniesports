...
let scrims = JSON.parse(localStorage.getItem("scrims")) || {}; // { '27-06-2025': [ { team2, time } ] }
...
function openModal(date, index = -1) {
  ...
  if (index >= 0) {
    const { team2, time } = scrims[date][index];
...
form.onsubmit = (e) => {
  ...
  if (editIndex >= 0) {
    scrims[date][editIndex] = { team2, time };
  } else {
    scrims[date].push({ team2, time });
  }
